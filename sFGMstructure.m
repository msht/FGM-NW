...
for ii=[1 2 4 10]
    ...
    for i = 0 : round(r_base/res)-1
     ...
     if i<= (round(r_base/res)-1)/2
         ...
         x=((1/2*(2*(i)/round(r_base/res))^ii));
        num_tbr = round((x/M_Ni)/(x/M_Ni+(1-x)/M_Cu)*num_temp_atom);
     else
         x=((1-1/2*(2*(1-(i)/round(r_base/res)))^ii));
       num_tbr = round((x/M_Ni)/(x/M_Ni+(1-x)/M_Cu)*num_temp_atom);
     end
    ...
    mkdir(['D:\Dropbox\Dropbox\codes_for_structures\vibration\sfgm\' char(string(ii)) '\']);
    mkdir(['D:\Dropbox\Dropbox\codes_for_structures\tension\sfgm\' char(string(ii)) '\']);
    for kk=1:2
        if (kk==1)
            filename = ['D:\Dropbox\Dropbox\codes_for_structures\TENSION\sfgm\' char(string(ii)) '\struct'];
        else
            filename = ['D:\Dropbox\Dropbox\codes_for_structures\vibration\sfgm\' char(string(ii)) '\struct'];
        end
