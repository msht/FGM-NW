...
for ii=[0.1 0.2 0.5 1 2 5 10]
    ...
    mkdir(['D:\Dropbox\Dropbox\codes_for_structures\vibration\pfgm\' char(string(ii)) '\']);
    mkdir(['D:\Dropbox\Dropbox\codes_for_structures\tension\pfgm\' char(string(ii)) '\']);
    for kk=1:2
        if (kk==1)
            filename = ['D:\Dropbox\Dropbox\codes_for_structures\TENSION\pfgm\' char(string(ii)) '\struct'];
        else
            filename = ['D:\Dropbox\Dropbox\codes_for_structures\vibration\pfgm\' char(string(ii)) '\struct'];
        end
